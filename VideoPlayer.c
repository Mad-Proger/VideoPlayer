...
#define FFMPEG_PIXEL_FORMAT AV_PIX_FMT_YUYV422
#define SDL_PIXEL_FORMAT SDL_PIXELFORMAT_YUY2

#define FFMPEG_SAMPLE_FORMAT AV_SAMPLE_FMT_FLT
#define SDL_SAMPLE_FORMAT AUDIO_F32
...
VideoPlayer* createVideoPlayer(VideoFile* vf) {
    ...
    vp->videoFrameQueue = createFrameQueue((size_t)av_q2d(vf->pFormatContext->streams[vf->pVideoStream->streamIndex]->r_frame_rate),
                                           FFMPEG_PIXEL_FORMAT, vp->renderRect.w, vp->renderRect.h);
    if (vp->videoFrameQueue == NULL) {
        destroyVideoPlayer(vp);
        return NULL;
    }

    vp->videoConvertContext = sws_getContext(vf->pVideoStream->pStreamCodecContext->width, vf->pVideoStream->pStreamCodecContext->height,
                                             vf->pVideoStream->pStreamCodecContext->pix_fmt,
                                             vp->renderRect.w, vp->renderRect.h, FFMPEG_PIXEL_FORMAT, SWS_BICUBIC, NULL, NULL, 0);
    if (vp->videoConvertContext == NULL) {
        destroyVideoPlayer(vp);
        return NULL;
    }

    vp->audioConvertContext = swr_alloc_set_opts(NULL, vf->pAudioStream->pStreamCodecContext->channel_layout, FFMPEG_SAMPLE_FORMAT,
                                                 vf->pAudioStream->pStreamCodecContext->sample_rate, vf->pAudioStream->pStreamCodecContext->channel_layout,
                                                 vf->pAudioStream->pStreamCodecContext->sample_fmt, vf->pAudioStream->pStreamCodecContext->sample_rate, 0, NULL);
...
void startVideoPlayer(VideoPlayer* vp) {
    ...
    vp->window = SDL_CreateWindow("Video player", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, vp->width, vp->height,
    ...
    vp->videoFrameTexture = SDL_CreateTexture(vp->renderer, SDL_PIXEL_FORMAT, SDL_TEXTUREACCESS_STREAMING, vp->renderRect.w, vp->renderRect.h);
    ...
    desired.format = SDL_SAMPLE_FORMAT;
...
int decodeAudioThreadVideoPlayer(void* data) {
    ...
    while (!checkEndedAudioQueue(vp->audioSampleQueue)) {
        ...
        if (sent == 0 && received == 0 && !vp->quit) {
            ...
            pDecodedSample->format = FFMPEG_SAMPLE_FORMAT;
            ...
            pushAudioQueue(vp->audioSampleQueue, pDecodedSample->data[0],
                           av_samples_get_buffer_size(pDecodedSample->linesize, pDecodedSample->channels,
                                                      pDecodedSample->nb_samples, FFMPEG_SAMPLE_FORMAT, 0),
                           pAudioSample->pts * audioBaseTime);
...
int decodeVideoThreadVideoPlayer(void* data) {
    ...
    pDecodedFrame->format = FFMPEG_PIXEL_FORMAT;
    ...
    while (!checkEndedFrameQueue(vp->videoFrameQueue)) {
        ...
        if (sent == 0 && received == 0 && !vp->quit) {
            sws_scale(vp->videoConvertContext, (const uint8_t* const*)pVideoFrame->data, pVideoFrame->linesize,
                      0, pVideoFrame->height, pDecodedFrame->data, pDecodedFrame->linesize);
